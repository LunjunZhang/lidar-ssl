...
class PointPillarsScatter(nn.Module):
    ...
    def forward(self, voxel_features, coords):
        ...
        if self.batch_size == 1:
            ...
            bev_canvas = torch.zeros(self.nx * self.ny)
            bev_canvas[second_indices] = 1.0
            bev_canvas = bev_canvas.view(self.ny, self.nx)
            import matplotlib.pyplot as plt
            plt.imshow(bev_canvas.numpy())
            plt.show()
            plt.savefig('bev_canvas' + '.png')
            ...
            raise NotImplementedError("PointPillarsScatter does not support scatter at the moment")
            # return 0
