...
class RPN(nn.Module):
    def __init__(
            ...
            aggregator_n_filters=[256, 128],
            projector_n_filters=[64, 64],
            predictor_n_filters=[64, 64],
        ...
        n_channels_concat = sum(num_upsample_filters)

        # Aggregator
        self.aggregator_block = Sequential(
            Conv2d(n_channels_concat, aggregator_n_filters[0], 3, padding=1),
            BatchNorm2d(aggregator_n_filters[0]),
            nn.ReLU(),
        )
        for i in range(1, len(aggregator_n_filters)):
            self.aggregator_block.add(Conv2d(aggregator_n_filters[i-1], aggregator_n_filters[i], 3, padding=1))
            self.aggregator_block.add(BatchNorm2d(aggregator_n_filters[i]))
            if i < len(aggregator_n_filters)-1:
                self.aggregator_block.add(nn.ReLU())
        
        rep_n_filters = aggregator_n_filters[-1]

        # Projector
        self.projector_module = Sequential(
            Conv2d(rep_n_filters, projector_n_filters[0], 1, padding=0),
            BatchNorm2d(projector_n_filters[0]),
            nn.ReLU(),
        )
        for i in range(1, len(projector_n_filters)):
            self.projector_module.add(Conv2d(projector_n_filters[i-1], projector_n_filters[i], 1, padding=0))
            self.projector_module.add(BatchNorm2d(projector_n_filters[i]))
            if i < len(projector_n_filters)-1:
                self.projector_module.add(nn.ReLU())
        
        # Predictor
        self.predictor_module = Sequential(
            Conv2d(projector_n_filters[-1], predictor_n_filters[0], 1, padding=0),
            BatchNorm2d(predictor_n_filters[0]),
            nn.ReLU(),
        )
        for i in range(1, len(predictor_n_filters)):
            self.predictor_module.add(Conv2d(predictor_n_filters[i-1], predictor_n_filters[i], 1, padding=0))
            self.predictor_module.add(BatchNorm2d(predictor_n_filters[i]))
            if i < len(predictor_n_filters)-1:
                self.predictor_module.add(nn.ReLU())

        if encode_background_as_zeros:
            num_cls = num_anchor_per_loc * num_class
        else:
            num_cls = num_anchor_per_loc * (num_class + 1)
        self.conv_cls = nn.Conv2d(rep_n_filters, num_cls, 1)
        self.conv_box = nn.Conv2d(
            rep_n_filters, num_anchor_per_loc * box_code_size, 1)
        if use_direction_classifier:
            self.conv_dir_cls = nn.Conv2d(
                rep_n_filters, num_anchor_per_loc * 2, 1)

    def forward(self, x, bev=None, stopgrad_before_eval=False):
        x = self.block1(x)
        up1 = self.deconv1(x)
        ...
        # now, get the representation
        x = self.aggregator_block(x)
        representation = x
        x = self.projector_module(x)
        projected_z = x
        x = self.predictor_module(x)
        predicted_p = x
        
        hidden_h = representation
        if stopgrad_before_eval:
            hidden_h = hidden_h.detach()
        box_preds = self.conv_box(hidden_h)
        cls_preds = self.conv_cls(hidden_h)
        # [N, C, y(H), x(W)]
        ...
        if self._use_direction_classifier:
            dir_cls_preds = self.conv_dir_cls(hidden_h)
            dir_cls_preds = dir_cls_preds.permute(0, 2, 3, 1).contiguous()
            # ret_dict["dir_cls_preds"] = dir_cls_preds
            ret_tuple += (dir_cls_preds,)
        # return ret_dict
        return ret_tuple, dict(
            representation=representation, projected_z=projected_z, predicted_p=predicted_p)
...
class RPN_v2(nn.Module):
    def __init__(
        ...
        n_channels_concat = num_filters[0]
        print('USING RPN_v2 ... ')
...
class VoxelNet(nn.Module):
    def __init__(
            ...
            rpn_aggregator_n_filters=[256, 128],
            rpn_projector_n_filters=[64, 64],
            rpn_predictor_n_filters=[64, 64],
        ...
        self.rpn = rpn_class(
            ...
            aggregator_n_filters=rpn_aggregator_n_filters,
            projector_n_filters=rpn_projector_n_filters,
            predictor_n_filters=rpn_predictor_n_filters,
    ...
    def forward_with_ssl(self, example, stopgrad_before_eval=False):
        pillar_x = example[0]
        pillar_y = example[1]
        pillar_z = example[2]
        pillar_i = example[3]
        ...
        voxel_features = self.voxel_feature_extractor(
            pillar_x, pillar_y, pillar_z, pillar_i, num_points, x_sub_shaped, y_sub_shaped, mask)
        ...
        preds_dict, ssl_dict = self.rpn(spatial_features, stopgrad_before_eval=stopgrad_before_eval)
        ...
        import pdb; pdb.set_trace()
        return output_tuple, ssl_dict
    
    def forward(self, example, stopgrad_before_eval=False):
