...
from second.core.point_cloud.point_cloud_ops_flow import single_sample_flow_3d_to_bev

from second.pytorch import flow_model
        ...
        for _ in range(total_loop):
            ...
            for step in range(steps):
                ...
                with torch.no_grad():
                    ...
                    s1_bev_flow = single_sample_flow_3d_to_bev(
                ...
                s2_f1_pts = s2_f1_pts.reshape(1, *s2_f1_pts.shape).contiguous()
                s2_f2_pts = s2_f2_pts.reshape(1, *s2_f2_pts.shape).contiguous()
                with torch.no_grad():
                    pred_flows, _, _, _, _ = flow_model.model(s2_f1_pts, s2_f2_pts, s2_f1_pts, s2_f2_pts, )
                    full_flow = pred_flows[0].permute(0, 2, 1)
                    s2_bev_flow = single_sample_flow_3d_to_bev(
                        points=s2_f1_pts[0].cpu().numpy(), 
                        scene_flow=full_flow[0].cpu().numpy(), 
                        voxel_size=voxel_generator.voxel_size,
                        coors_range=voxel_generator.point_cloud_range,
                        max_points=voxel_generator.max_num_points_per_voxel,
                ...
                import flow_vis
                x = flow_vis.flow_to_color(s2_bev_flow, convert_to_bgr=False)
                import matplotlib.pyplot as plt
                plt.imshow(x)
                plt.show()
                plt.savefig('x_flow_' + str(step) + '.png')
                plt.close()
                import open3d as o3d
                pcd = o3d.geometry.PointCloud()
                pcd.points = o3d.utility.Vector3dVector(s2_f1_pts[0].cpu().numpy())
                o3d.io.write_point_cloud("pcd_" + str(step) + ".ply", pcd)
