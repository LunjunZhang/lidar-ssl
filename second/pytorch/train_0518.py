...
def train(
        ...
        use_online_target=True,
        seq_kitti_info_path="/h/lunjun/KITTI_TRACKING/kitti_infos_train.pkl",
        seq_kitti_root_path="/h/lunjun/KITTI_TRACKING",
    ):
    """train a VoxelNet model specified by a config file.
    """
    print('use_online_target', use_online_target)
    print('polyak', polyak)
    print('update_targ_freq', update_targ_freq)
    print('byol_scale', byol_scale)
    ...
    try:
        for total_loop_step in range(total_loop):
            ...
            for step in range(steps):
                ...
                batch_size = seq_example_torch["anchors"].shape[0]
                # print('batch_size:', 2)

                # for the target network 
                frame1_input = prepare_net_input(seq_example_torch, key_suffix='')
                with torch.no_grad():
                    net_for_projection = net if use_online_target else net_target
                    net_for_projection.eval()
                    _, frame1_ssl_dict = net_for_projection.forward_with_ssl(
                        frame1_input, stopgrad_before_eval=stopgrad_before_eval)
                frame1_project = frame1_ssl_dict['projected_z']
                
                # for the online network 
                frame2_input = prepare_net_input(seq_example_torch, key_suffix=NEXT_1_suffix)
                net.train()
                _, frame2_ssl_dict = net.forward_with_ssl(
                    frame2_input, stopgrad_before_eval=stopgrad_before_eval)
                frame2_predict = frame2_ssl_dict['predicted_p']
                ...
                with torch.no_grad():
                    s1_flow_1to2, s1_flow_2to1 = get_flow_3d(s1_f1_pts, s1_f2_pts)
                    s1_bev = single_sample_flow_to_bev_v3(
                        pts_1=s1_f1_pts.cpu().numpy(), 
                        pts_2=s1_f2_pts.cpu().numpy(), 
                        flow_1to2=s1_flow_1to2.cpu().numpy(), 
                        flow_2to1=s1_flow_2to1.cpu().numpy(), 
                ...
                with torch.no_grad():
                    s2_flow_1to2, s2_flow_2to1 = get_flow_3d(s2_f1_pts, s2_f2_pts)
                    s2_bev = single_sample_flow_to_bev_v3(
                        pts_1=s2_f1_pts.cpu().numpy(), 
                        pts_2=s2_f2_pts.cpu().numpy(), 
                        flow_1to2=s2_flow_1to2.cpu().numpy(), 
                        flow_2to1=s2_flow_2to1.cpu().numpy(), 
                        voxel_size=voxel_generator.voxel_size,
                        coors_range=voxel_generator.point_cloud_range,
                        max_points=voxel_generator.max_num_points_per_voxel,
                        reverse_index=False,
                ...
                '''
                batch_bev_flow = np.stack([s1_bev["bev_flow_1to2"], s2_bev["bev_flow_1to2"]], axis=0)
                torch_bev_flow = torch.from_numpy(batch_bev_flow).cuda()
                torch_bev_flow = torch_bev_flow.permute(0, 3, 1, 2)

                frame1_project = tf.interpolate(frame1_project, scale_factor=2, mode='bilinear')
                frame1_warped = flow_model.warp_features(frame1_project, torch_bev_flow)
                frame2_predict = tf.interpolate(frame2_predict, scale_factor=2, mode='bilinear')
                
                # normalize
                frame1_normalized = tf.normalize(frame1_warped, dim=1)
                frame2_normalized = tf.normalize(frame2_predict, dim=1)
                # [B, C, H, W]
                batch_bev_mask = np.stack([s1_bev["bev_img_2"], s2_bev["bev_img_2"]], axis=0)
                torch_bev_mask = torch.from_numpy(batch_bev_mask).cuda()
                masked_cosine = (frame1_normalized * frame2_normalized).sum(dim=1) * torch_bev_mask
                byol_loss = -2.0 * masked_cosine.sum() / torch_bev_mask.sum()
                print('byol_loss:', byol_loss.item())
                byol_loss_list.append(byol_loss.item())
                ...
                if global_step % display_step == 0:
                    ...
                    '''
                    metrics["loss"]["byol"] = float(np.mean(byol_loss_list))
                    byol_loss_list = []
                    metrics["mask_sum"] = float(torch_bev_mask.sum().detach().cpu().numpy())
