...
from second.data.seq_preprocess import read_and_prep_start_end, read_single_frame
...
class TrackingDataset(Dataset):
    def __init__(
        ...
        self._prep_func = partial(prep_func, anchor_cache=anchor_cache)
        self._length_per_segment = length_per_segment
    
    def __len__(self):
        # return len(self._kitti_infos)
        return len(self._kitti_infos) * 1000
    ...
    def __getitem__(self, idx):
        idx = idx % len(self._kitti_infos)
        return read_and_prep_start_end(
            seq_info=self._kitti_infos[idx],
            root_path=self._root_path,
            num_point_features=self._num_point_features,
            prep_func=self._prep_func, 
            length_per_segment=self._length_per_segment,
        )
    
    def get_single_frame(self, idx, frame_idx):
